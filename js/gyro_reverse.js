...
var ball = Bodies.circle(w/2, w/2, Common.random(20, 30), {
                render: {
                    sprite: {
                        texture: 'img1.jpeg',
                        xScale: 0.3,
    ...
    var updateGravity = function(event) {
        ...
        if (orientation === 0) {
            gravity.x = -1*Common.clamp(event.gamma, -90, 90) / 90;
            gravity.y = -1*Common.clamp(event.beta, -90, 90) / 90;
        } else if (orientation === 180) {
            gravity.x = -1*Common.clamp(event.gamma, -90, 90) / 90;
            gravity.y = -1*Common.clamp(-event.beta, -90, 90) / 90;
        } else if (orientation === 90) {
            gravity.x = -1*Common.clamp(event.beta, -90, 90) / 90;
            gravity.y = -1*Common.clamp(-event.gamma, -90, 90) / 90;
        } else if (orientation === -90) {
            gravity.x = -1*Common.clamp(-event.beta, -90, 90) / 90;
            gravity.y = -1*Common.clamp(event.gamma, -90, 90) / 90;
        }
