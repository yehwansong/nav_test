...
var yellow_obs = []
    ...
    // red
        ...
        ['rec', 1,   4, 6.4,    1,          2, 5],
        ['rec', 1,  10,   4,    5.5,        2, 5],
        ['rec', 10.5, 8.2,      1.8, 1.5,   2, 5],
        ['rec', 14,      7,     1.2, 2.5,   2, 5],
        ...
        ['cir', 2,     10,   2,   0, 0],
        ['svg',5,  10, skew_cir ,   14, 0],
        ['svg',7,  10, skew_cir ,   14, 0 , 'rot'],
    ]
    ...
    function adding_obj(elem, sec, spec){
        ...
        // setTimeout(function(){World.add(world, elem);}, sec*00);
    }
        ...
        for (var i = 0; i < chain_3.length ; i++) {
            ...
            if(i == chain_3.length-1){
                ...
                for (var j = chain_array_3.length - 1; j >= 0; j--) {
                    adding_obj(chain_array_3[j],0)
                }
                for (var j = constraint_array_3.length - 1; j >= 0; j--) {
                    adding_obj(constraint_array_3[j],0)
                }
            }
        }
//extra
    ...
    World.add(world, [color, wall1, wall2])
...
function detectcollision(){
    ...
    setTimeout(function(){reset_field_blue()}, 22000);
    setTimeout(function(){reset_field_red()}, 26000);
    setTimeout(function(){reset_field_yellow()}, 30000);
    function reset_field_blue(){
        setTimeout(function(){reset_field_blue()}, 22000);
            for (var i = blue_obs.length - 1; i >= 0; i--) {
                Matter.Composite.remove(world, blue_obs[i])
            }
            adding_blue() 
    }
    function reset_field_red(){
        setTimeout(function(){reset_field_red()}, 22000);
            for (var i = red_obs.length - 1; i >= 0; i--) {
                Matter.Composite.remove(world, red_obs[i])
            }
            adding_red() 
    }
    function reset_field_yellow(){
        setTimeout(function(){reset_field_yellow()}, 22000);
            for (var i = yellow_obs.length - 1; i >= 0; i--) {
                Matter.Composite.remove(world, yellow_obs[i])
            }
            adding_yellow() 
    }
