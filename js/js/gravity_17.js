...
$('document').ready(function(){
  for (var i = 0; i < $('input').length; i++) {
    console.log('hey')
    $('input').eq(i).val(value_array_n[i])
  }
})
}else{
    var blue_fric = 0.1
...
var chain_wrap = 
    ...
    var chain_3=[
        ...
        for (var i = 0; i < chain_3[0].length ; i++) {
            chain_array.push(Bodies.circle((chain_3[0][i][1]+(chain_3[0][i][3]/2))*50, (chain_3[0][i][2]+(chain_3[0][i][3]/2))*50 - offset, (chain_3[0][i][3]/2*50), render_setting[chain_3[0][i][4]]))
            if(i>0){
                constraint_array.push(
                    Constraint.create({ 
                        bodyA: chain_array[i],
            ...
            if(i == chain_wrap[0].length-1){
                console.log(chain_array)
                for (var j = chain_array.length - 1; j >= 0; j--) {
                    adding_obj(chain_array[j],blue_sec)
                }
                for (var j = constraint_array.length - 1; j >= 0; j--) {
                    adding_obj(constraint_array[j],blue_sec)
                }
            }
        }
//extra
